...
if (($_SESSION['role'] == 1) || ($_SESSION['role'] == 3)) {
    ...
    header("Location: sohbetler-okul-izle.php");
    exit();
}
...
<body id="kt_app_body" data-kt-app-header-fixed="true" data-kt-app-header-fixed-mobile="true"
    ...
    <script>
        ...
        async function loadConversations() {
            try {
                const response = await fetch('includes/chat_school_handler.inc.php?action=get_conversations');
            ...
            <div class="d-flex align-items-center">
                ...
                <div class="flex-grow-1">
                    <div class="d-flex align-items-center justify-content-between mb-1">
                        <span class="text-gray-900 fw-bold fs-6">${conv.other_name} ${conv.other_surname} ${(conv.lessonName && isTeacher()) ? '(' + conv.lessonName + ')' : ''}</span>
                        ${conv.unread_count > 0 ? `<span class="unread-badge">${conv.unread_count}</span>` : ''}
                    </div>
                    <span class="text-gray-500 fw-semibold fs-7 d-block">
                        ${conv.last_message ? (conv.last_message.length > 30 ? conv.last_message.substring(0, 30) + '...' : conv.last_message) : 'Dosya gönderildi'}
                    </span>
                </div>
            </div>
        </div>
    `).join('');
        ...
        function loadMessages() {
            ...
            fetch(`includes/chat_school_handler.inc.php?action=get_messages&conversation_id=${currentConversationId}`)
    ...
    `;
        ...
        <li class="list-group-item contact-item" onclick="startConversation(${user.id}, '${user.username}','${user.photo}')">
            <div class="d-flex align-items-center">
                ...
                <div>
                    <div class="fw-bold">${user.name} ${user.surname} (${user.lessonName})</div>
                </div>
            </div>
        </li>
    `).join('');
        ...
        function isTeacher() {
            return <?php echo $_SESSION['role'] == 4; ?>;
        }
    </script>
