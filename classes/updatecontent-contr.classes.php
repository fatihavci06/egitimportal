...
class UpdateContentContr extends UpdateContent
{
...
class UpdateContentVideoContr extends UpdateContent
{
	...
	public function __construct($title, $summary, $contentId, $video_url, $videoId, $photoSize, $photoName, $fileTmpName)
	{
		$this->photoSize = $photoSize;
		$this->photoName = $photoName;
		$this->fileTmpName = $fileTmpName;
	...
	public function updateContentVideoDb()
	{

		$contents = new GetContent();
		$contentInfo = $contents->getAllContentDetailsById($this->contentId);

		if ($this->title != $contentInfo['title']) {
		...
		if ($this->fileTmpName != NULL) {
			...
			$imgName = $contentInfo['cover_img'];
		}

		$this->updateContent($imgName, $slug, $this->title, $this->summary, $this->contentId, $this->video_url, $this->videoId);
	}
}