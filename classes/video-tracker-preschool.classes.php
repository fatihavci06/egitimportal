...
class VideoTracker
{
    ...
    public function store($userId, $videoId, $timestampSeconds)
    {
        try {
            ...
            INSERT INTO video_timestamp_preschool_lnp (user_id, video_id, timestamp_in_seconds) 
            VALUES (?, ?, ?)
    ...
    public function saveDuration($video_id, $duration)
    {
        try {
            $stmt = $this->db->prepare("
            INSERT INTO video_durations_preschool (video_id, duration) 
            VALUES (?, ?)
            ON DUPLICATE KEY UPDATE 
    ...
    public function getWatchEvents($userId, $videoId)
    {
        try {
            $stmt = $this->db->prepare("
                ...
                FROM video_timestamp_preschool_lnp
