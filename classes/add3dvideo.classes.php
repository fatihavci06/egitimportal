...
class AddContent extends Dbh
{
    ...
    protected function setContent($imgName, $slug, $name, $classes, $short_desc, $video_url)
    {
        $pdo = $this->connect();
        ...
        try {
            ...
            // INSERT INTO 3dsciencevideos_lnp
            $stmt = $pdo->prepare('INSERT INTO 3dsciencevideos_lnp SET slug=?, title=?, summary=?, cover_img=?, videoUrl=?');
            if (!$stmt->execute([$slug, $name, $short_desc, $imgName, $video_url])) {
                // Hata oluştuğunda Exception fırlat
                throw new Exception("Ana içerik eklenirken veritabanı hatası."); //
            }

            $mainId = $pdo->lastInsertId();

            // INSERT INTO 3dsciencevideosclasses_lnp
            $stmtClass = $pdo->prepare("INSERT INTO 3dsciencevideosclasses_lnp SET video_id=?, class_id=?");
            $classes = explode(",", $classes); // Sınıfları ayır

            foreach ($classes as $class) {
                if (!$stmtClass->execute([$mainId, $class])) {
                    // Hata oluştuğunda Exception fırlat
                    throw new Exception("Sınıf ilişkisi eklenirken veritabanı hatası."); //
                }
            }
            
            $pdo->commit(); // İşlemleri onayla
            echo json_encode(["status" => "success", "message" => $name]);

        } catch (\Exception $e) {
            $pdo->rollback(); // Hata durumunda işlemleri geri al
            // Hata mesajını Exception'dan alıp döndür
    ...
    {
        $stmt = $this->connect()->prepare('SELECT slug FROM 3dsciencevideos_lnp WHERE slug LIKE ? OR slug = ? ORDER BY slug ASC');
    ...
    protected function setContent($imgName, $slug, $name, $classes, $lessons, $units, $short_desc, $topics, $sub_topics, $content, $video_url, $file_urls, $imageFiles, $descriptions, $titles, $urls)
    {
        ...
        try {
            ...
            if (!empty($video_url)) {
                ...
                $stmtClass = $pdo->prepare("INSERT INTO school_content_videos_url SET school_content_id=?, video_url=?");


                if (!$stmtClass->execute([$mainId, $video_url])) {
                    $stmtClass = null;
                    //header("location: ../admin.php?error=stmtfailed");
                    exit();
                }
            ...
            if (!empty($file_urls)) {
                ...
                foreach ($file_urls as $index => $url) {
                    ...
                    $stmtClass = $pdo->prepare("INSERT INTO school_content_files_lnp (school_content_id, file_path, description) VALUES (:school_content_id, :file_path, :description)");
                    $stmtClass->execute([
            ...
            if (isset($titles) && isset($urls)) {
                ...
                for ($i = 0; $i < count($titles); $i++) {
                    ...
                    if ($title !== '' && $url !== '') {
                        $stmtClass = $pdo->prepare("INSERT INTO school_content_wordwall_lnp (school_content_id, wordwall_title,wordwall_url) VALUES (:school_content_id, :wordwall_title ,:wordwall_url)");
                        $stmtClass->execute([
                            ':school_content_id' => $mainId,
