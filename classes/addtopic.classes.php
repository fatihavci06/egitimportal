...
class AddSubTopic extends Dbh
{
	...
	{
		$stmt = $this->connect()->prepare('INSERT INTO subtopics_lnp SET slug = ?, name = ?, class_id = ?, lesson_id = ?, unit_id = ?, topic_id = ?, school_id=?, teacher_id=?, short_desc=?, image=?, start_date=?, end_date=?, order_no=?');
		...
		if (!$stmt->execute([$slug, $name, $classes, $lessons, $units, $topics, $school, $teacher, $short_desc,  $imgName, $start_date, $end_date, $order])) {
			$stmt = null;
			//header("location: ../admin.php?error=stmtfailed");
			exit();
		}
