...
class Support extends Dbh
{
	...
	public function getSupport($userID)
	{
		...
		$stmt = $this->connect()->prepare('SELECT
			...
			INNER JOIN
			users_lnp u ON dt.writer = u.id WHERE dt.openedBy =? AND dt.completed =?');
	...
	public function getSupportAdmin()
	{
		...
		$stmt = $this->connect()->prepare('SELECT
			...
			ON dt.slug = son_talepler.slug AND dt.created_at = son_talepler.son_olusturma
			INNER JOIN
			users_lnp u ON dt.writer = u.id WHERE dt.completed =?');
	...
	{
		$stmt = $this->connect()->prepare('SELECT support_center_lnp.id, support_center_lnp.slug, support_center_lnp.subject, support_center_lnp.title, support_center_lnp.comment, support_center_lnp.image, support_center_lnp.writer, support_center_lnp.openedBy, support_center_lnp.completed, support_center_lnp.created_at AS created_at, support_center_lnp.updated_at AS updated_at, users_lnp.name AS userName, users_lnp.surname AS userSurname, users_lnp.photo AS photo FROM support_center_lnp INNER JOIN users_lnp ON support_center_lnp.writer = users_lnp.id WHERE support_center_lnp.openedBy = ? AND support_center_lnp.slug = ? ORDER BY support_center_lnp.created_at DESC');
	...
	{
		$stmt = $this->connect()->prepare('SELECT support_center_lnp.id, support_center_lnp.slug, support_center_lnp.subject, support_center_lnp.title, support_center_lnp.comment, support_center_lnp.image, support_center_lnp.writer, support_center_lnp.openedBy, support_center_lnp.completed, support_center_lnp.created_at AS created_at, support_center_lnp.updated_at AS updated_at, users_lnp.name AS userName, users_lnp.surname AS userSurname, users_lnp.photo AS photo FROM support_center_lnp INNER JOIN users_lnp ON support_center_lnp.writer = users_lnp.id WHERE support_center_lnp.slug = ? ORDER BY support_center_lnp.created_at DESC');
