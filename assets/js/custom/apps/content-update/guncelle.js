...
var KTModalCustomersAdd = function () {
			...
			form,
			{
				fields: {
					...
					'start_date': {
						validators: {
							notEmpty: {
								message: 'Alt Konu Başlangıç Tarihi zorunlu'
							}
						}
					},
					'end_date': {
						validators: {
							notEmpty: {
								message: 'Alt Konu Bitiş Tarihi zorunlu'
							}
						}
					},
					'order': {
						validators: {
							notEmpty: {
								message: 'Alt Konu Sırası zorunlu'
							}
						}
					},
				},
				...
				validator.validate().then(function (status) {
					...
					if (status == 'Valid') {
						...
						setTimeout(function () {
							...
							const getContentId = document.getElementById('title');
							var contentId = getContentId.dataset.fileId;
							formData.append('contentId', contentId);

							if (video_url) {
								var videoId = video_url.dataset.fileId;
								formData.append('videoId', videoId);
								var action = 'video_url';
							} else {
							...
							$.ajax({
								type: "POST",
								url: "includes/updatecontent.inc.php?action=" + action,
								data: formData,
								contentType: false,
								processData: false,
								/*data: {
								...
								success: function (response) {
									if (response.status === "success") {
										...
										Swal.fire({
											text: response.message + " adlı içerik güncellenmiştir!",
											icon: "success",
											buttonsStyling: false,
											confirmButtonText: "Tamam, anladım!",
											customClass: {
	...
	return {
		...
		init: function () {
			...
			submitButton = form.querySelector('#submitForm');
